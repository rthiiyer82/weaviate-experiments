...
import {
  addClassToSchema,
  addObjectsToSchema,
  addObjectsWithDeterministicID,
  addObjectsWithID,
  addObjectsWithVector,
...
const SCENARIO_TAG = "@create-object";
...
defineFeature(feature, (test) => {
  ...
  const andCollectionCreated = (and: DefineStepFunction): void => {
    and("I have a collection", async () => {
      await addClassToSchema(classWithProps);
    });
  };

  test("Create an Object in a class", ({ given, when, then, and }) => {
    givenInstanceCreated(given);
    andCollectionCreated(and);

    when(/^I add objects to the collection$/, async () => {
      await addObjectsToSchema(classWithProps.class, objectPropValue.title, objectPropValue.genres,
        objectPropValue.keyword, objectPropValue.cast);
    });
    ...
    then(
      ...
      async () => {
        await waitForAssertion(async () => {
          const client = await connectWeaviateInstanceLocalHost();
          const response = await client.graphql.get().withClassName(classWithProps.class).withFields('title genres language director revenue cast').do();
          const result: typeof objectPropValue = response.data.Get[classWithProps.class];
          expect(result.cast).not.toBeNull;
  ...
  test("Create an Object with vector", ({ given, when, then, and }) => {
    givenInstanceCreated(given);
    andCollectionCreated(and);

    when(/^I add objects with vector to a class$/, async () => {
      await addObjectsWithVector(classWithProps.class, objectPropValue.title);
    });
  ...
  test("Create an Object with Id", ({ given, when, then, and }) => {
    givenInstanceCreated(given);
    andCollectionCreated(and);

    when(/^I add objects with ID to a class$/, async () => {
      await addObjectsWithID(classWithProps.class, objectPropValue.title);
    });
  ...
  test("Create an object with determinstic ids", ({ given, when, then, and }) => {
    givenInstanceCreated(given);
    andCollectionCreated(and);

    when(/^I add objects with determistic ID to a class$/, async () => {
      await addObjectsWithDeterministicID(classWithProps.class, objectPropValue.title);
    });
